...
export function badgeSvg(label: string, message: string): string {
...
<rect x="${leftWidth}" width="${rightWidth}" height="20" fill="${GRAPH_ACCENT}"/>
...
export function badgeResponse(format: BadgeFormat, badge: BadgeResponse): Response {
	return format === "json"
		? Response.json(badge)
		: new Response(badgeSvg(badge.label, badge.message), {
